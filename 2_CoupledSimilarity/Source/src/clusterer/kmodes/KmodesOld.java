 ...
 */
public class KmodesOld extends AAI_Module implements Clusterer {
	...
	public void kModes(int k, DataSet data, SimMeasure simMeasure, boolean random) throws Exception {
		output(k + "-modes algorithm started on data set " + Common.quote(data.m_dataName));
		output(data.objNum() + " objects.");
		...
		for (int changeNum = 1; changeNum > 0; roundNum++) {
			...
			output("round:" + (roundNum + 1) + ", " + changeNum + " changes.");
		}
		output(k + "-modes algorithm finished. " + roundNum + " iterations. " + timer);
	}
	...
	protected void mainProc() throws Exception {
		...
		KmodesOld kmodes = new KmodesOld();
		kmodes.debug(true);
