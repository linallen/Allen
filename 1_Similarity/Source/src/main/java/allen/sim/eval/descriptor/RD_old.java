 ...
 * intra-cluster similarity for all cluster labels.<br>
 * Higher better
 * 
 * RD(simMeasure, dataSet) = average(intra-cluster similarity) /
 * average(inter-cluster similarity)
 * 
 * @author Allen Lin, 29 Mar 2016
 */
public class RD_old extends Descriptor {
	...
	public double getMetric(SimMeasure simMeasure, DataSet dataSet) throws Exception {
		...
		return (interAve == 0) ? 0 : (intraAve / interAve);
	}
}