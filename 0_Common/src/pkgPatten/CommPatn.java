...
public class CommPatn {
	...
	public static String factorStr(String ftrName, String ftrValue)
			throws Exception {
		if (ftrName.trim().isEmpty() || ftrValue.trim().isEmpty()) {
			throw new Exception("empty feature name or value: " + ftrName + "="
					+ ftrValue);
		}
	...
	public static HashSet<String> getFtrFactors(String ftrName, Arff arffHdr)
			throws Exception {
	...
	public static String[] getFactors(String usrFactors[], Arff arffHdr)
			throws Exception {
		HashSet<String> factors = new HashSet<String>();
		for (String usrFactor : usrFactors) {
			...
			if (items.length == 1) {
				...
				throw new Exception("Wrong factor format: "
						+ Common.quote(usrFactor));
			}
