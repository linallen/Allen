...
import pkgFeature.Feature;
import pkgFeature.FtrType;
import pkgModule.AAI_Module;
import pkgPatten.CommPatn;

/**
 * Arff class for manipulating Arff objects, including loading and saving ARFF,
 * alerting, splitting, swapping, and deleting features, etc.
 * 
...
public class Arff extends AAI_Module {
	 ...
	 * 
	 * @param clsOpt
	 *            might be class index or class name
	 * @return index of class (>= 0, or < 0 if not found)
	 */
	public static int getClsIdx(String arffFile, String clsOpt)
			throws Exception {
		int clsIdx = -1;
		...
		if (Common.isInteger(clsOpt)) {
			...
			if (clsIdx > arff.ftrNum() - 1) {
				throw new Exception("Class index out of bound. "
						+ Common.quote(clsOpt));
			}
		...
		if (arff.getFeature(clsIdx).type() != FtrType.CATEGORICAL) {
			throw new Exception("Class is not CATEGORICAL. "
					+ Common.quote(arff.getFeature(clsIdx).name() + " ("
							+ clsOpt + ")"));
		}
		return clsIdx;
	}
	...
	private static void readFtrValues(Feature ftr, String line)
			throws Exception {
	...
	public String[] splitArff(String arffFile, String ftrName) throws Exception {
		...
		try {
			...
			for (int i = 0; i < splitFtr.ftrValueNum(); i++) {
				subArffs[i] = arffFile + "(" + ftrName + "="
						+ splitFtr.getFtrValue(i) + ").arff";
	...
	public String[] getFtrFactors(String ftrName) throws Exception {
		...
		for (int i = 0; i < ftr.ftrValueNum(); i++) {
			...
			factors[i] = CommPatn.factorStr(ftr.name(), ftrValue);
		}
		return factors;
	}

	/** "ftr1,ftr2" --> [["ftr1=A","ftr1=B"],["ftr2=C","ftr2=D"]] */
